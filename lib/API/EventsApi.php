...
class EventsApi
{
    ...
    public function bulkCreateEventsRequest($events_bulk_create_job, $apiKey = null, string $contentType = self::contentTypes['bulkCreateEvents'][0])
    {
        ...
        if (isset($events_bulk_create_job)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false || stripos($headers['Content-Type'], 'application/vnd.api+json') !== false) {
                ...
                $httpBody = ObjectSerializer::jsonEncode(ObjectSerializer::sanitizeForSerialization($events_bulk_create_job));
            } else {
    ...
    public function createEventRequest($event_create_query_v2, $apiKey = null, string $contentType = self::contentTypes['createEvent'][0])
    {
        ...
        if (isset($event_create_query_v2)) {
            if (stripos($headers['Content-Type'], 'application/json') !== false || stripos($headers['Content-Type'], 'application/vnd.api+json') !== false) {
                # if Content-Type contains "application/json" or "application/vnd.api+json", json_encode the body
                $httpBody = ObjectSerializer::jsonEncode(ObjectSerializer::sanitizeForSerialization($event_create_query_v2));
            } else {
    ...
    public function getEventsRequest($fields_attribution = null, $fields_event = null, $fields_metric = null, $fields_profile = null, $filter = null, $include = null, $page_cursor = null, $page_size = 200, $sort = null, $apiKey = null, string $contentType = self::contentTypes['getEvents'][0])
    {
        ...
        if ($page_size !== null && $page_size > 1000) {
            throw new \InvalidArgumentException('invalid value for "$page_size" when calling EventsApi.getEvents, must be smaller than or equal to 1000.');
        }
        if ($page_size !== null && $page_size < 1) {
            throw new \InvalidArgumentException('invalid value for "$page_size" when calling EventsApi.getEvents, must be bigger than or equal to 1.');
        }
